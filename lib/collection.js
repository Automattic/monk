...
Collection.prototype.insertOne = function (data, opts, fn) {
  if (typeof opts === 'function') {
    fn = opts
    opts = {}
  }

  return this._dispatch(function insertOne (args) {
    return args.col.insertOne(args.data, args.options).then(function (docs) {
      var res = (docs || {}).ops
      if (res) {
        res = docs.ops[0]
      }
  ...
  })({data: data, options: opts, callback: fn}, 'insertOne')
}

Collection.prototype.insertMany = function (data, opts, fn) {
  if (typeof opts === 'function') {
    fn = opts
    opts = {}
  }

  return this._dispatch(function insertMany (args) {
    if (args.data.length === 0) {
      return Promise.resolve([])
    }
    return args.col.insertMany(args.data, args.options).then(function (docs) {
      var res = (docs || {}).ops
      return res
    })
  })({data: data, options: opts, callback: fn}, 'insertMany')
}

Collection.prototype.bulkWrite = function (data, opts, fn) {
  if (typeof opts === 'function') {
    fn = opts
    opts = {}
  }
  ...
  return this._dispatch(function bulkWrite (args) {
    ...
    return args.col.bulkWrite(args.data, args.options).then(function (docs) {
      var res = (docs || {}).ops
      return res
    })
  })({data: data, options: opts, callback: fn}, 'bulkWrite')
}

...
var DEFAULT_OPTIONS = {
  ...
  middlewares: [
    ...
    require('../plugins/castIds')(FIELDS_TO_CAST),
    require('../plugins/fields')
  ]
}

function wrapperMiddleware (context) {
  return function (next) {
    return function (args) {
      return context.monkInstance.executeWhenOpened().then(function (db) {
        ...
        args.col = col
        return next(args)
      }).then(thenFn(args.callback)).catch(catchFn(args.callback))
    }
  }
}
...
function Collection (manager, name, options) {
  ...
  this.middlewares = (this.options.middlewares || []).concat(wrapperMiddleware)

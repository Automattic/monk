...
function Collection (manager, name, options) {
  ...
  this.index = this.ensureIndex = this.createIndexes = this.createIndexes.bind(this)
...
Collection.prototype.createIndexes = function (fields, opts, fn) {
  if (typeof opts === 'function') {
    fn = opts
    opts = {}
  }
  ...
  return this.executeWhenOpened().then(function (col) {
    return col.createIndexes(fields, opts)
  }).then(thenFn(fn)).catch(catchFn(fn))
}

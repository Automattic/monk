...
Collection.prototype.mapReduce = function (col, map, reduce, finalize, opts, fn) {
  if (typeof opts === 'function') {
    fn = opts
    opts = {}
  }
  ...
  // query
  debug('%s mapreduce %j,%j to %s', this.name, map, reduce, col);
  return this.executeWhenOpened().then(function (col) {
      return col.mapReduce(map, reduce, args);
  }).then(thenFn(fn)).catch(catchFn(fn))
}

...
/**
 * Performs a map reduce on the collection. Can also delete enteries in the current collection that were feed into the map reduce. All options are passed through to the mongo.
 *
 * https://docs.mongodb.com/manual/reference/command/mapReduce
 *
 * @param {String} col, new collection to dump the data too. A default can be provided
 * @param {Function} map, mapper function
 * @param {Function} reduce, reduce function
 * @param {Function} finalize, finalize function. A default can be provided.
 * @param {Object} [args], options or fields
 * @param {Boolean} [remove], Remove objects fed into map reduce function.
 * @param {Object|String|Array} [opts], options or fields
 * @param {Function} [fn] callback
 *
 * @example
 *
 * db.mapReduce("reduce_from_monk", function() { 
 *   emit(this.url, {"mean": +this.dltime, "median": 0}); 
 * }, function(url, dltimes) { 
 *   var mean = 0;
 *   var arr = [];
 *       for (var i = 0; i < dltimes.length; i++) {
 *           mean += dltimes[i].mean;
 *           if (arr.length < 25) {
 *               arr.push(dltimes[i].mean);
 *           } else {
 *               arr[i % 25] = dltimes[i].mean;
 *           }
 *       }
 *       mean = mean / dltimes.length;
 *       return {"mean": mean,"median": arr[Math.floor(arr.length / 2)]}
 *   }, function(k, v) { v.mean = v.mean.toFixed(2); return v; }, 
 *   {'verbose': true, 'inline': true, 'query': {'url': 'http://de9b7h88wgj5l.cloudfront.net/_default_/live4_hls/livestream.m3u8'}},
 *   true)
 * @return {Promise}
 */

Collection.prototype.mapReduce = function (col, map, reduce, finalize, args, remove, opts, fn) {
  if (typeof opts === 'function') {
    fn = opts
    opts = {}
  }
  ...
  args = args || {}
  args['out'] = col || 'reduce';
  args['finalize'] = finalize || 'function(k,v) { return v; }';
  remove = remove || false;

  // query
  debug('%s mapreduce %j,%j,%j to %s', this.name, map, reduce, args, col);
  var self = this;
  return this.executeWhenOpened().then(function (col) {
      // this will read lock.
      var promise = col.mapReduce(map, reduce, args);
      if (remove) {
